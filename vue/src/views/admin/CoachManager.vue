  ...
  <div>
    <div>
      <el-card style="margin-bottom: 10px">
        <el-input style="width: 240px" v-model="data.coachName" placeholder="请输入……" prefix-icon="Search"></el-input>
    ...
    <div class="card" style="margin-bottom: 5px">
      <el-table :data="data.tableData" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column label="姓名" prop="coachName" width="100px"/>
        <el-table-column label="身份" prop="role" width="100px"/>
        <el-table-column label="账号" prop="account" width="100px"/>
        <el-table-column label="手机号码" prop="phone" width="150px"/>
        ...
        <el-table-column label="操作" width="120">
          <template #default="scope">
            ...
            <el-button @click="del(scope.row.coachId)" :icon="Delete" circle type="danger"></el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="用户信息" v-model="data.formVisible" width="500" destroy-on-close>
      <el-form ref="formRef" :rules="data.rules" :model="data.form" label-width="80px" style="padding-right: 40px; padding-top: 20px">
        ...
        <el-form-item label="用户名" prop="coachName">
          <el-input v-model="data.form.coachName" autocomplete="off" placeholder="请输入名称" />
        </el-form-item>
        <el-form-item label="账号" prop="account">
          <el-input v-model="data.form.account" autocomplete="off" placeholder="请输入用户名" />
        </el-form-item>
...
const load = () => {
  request.get('/coach/selectPage',{ //这样才能发送这样的请求 ?pageNum=1&pageSize=5
    params: {
...
const save = () => { //一个保存方法中有两种操作 一个新增 一个编辑
  formRef.value.validate((valid)=> {
    if (valid) {
      data.form.coachId ? update() : add()
    }
  })
}

const add = () => {
  request.post('/coach/add', data.form).then(res => { //新增的对象无id
    if(res.code === '200'){
...
const update = () => {
  request.put('/coach/update',data.form).then(res => { //编辑的对象有id
    if(res.code === '200'){
      data.formVisible = false
      ElMessage.success('操作成功')
      load() //更新后要重新加载最新数据
...
const del = (coachId) => {
  ElMessageBox.confirm('删除数据后无法恢复，您确认删除嘛？','删除确认', {type:"warning"}).then(() => {
    request.delete('/coach/deleteById/' + coachId).then(res => {
      if(res.code === '200'){
        ElMessage.success('操作成功')
        load() //删除后要重新加载最新数据
      } else {
...
const handleSelectionChange = (rows) => { //返回所有返回的行对象数组
  ...
  data.ids = rows.map(row => row.coachId)
...
const delBatch = () => {
  ...
  ElMessageBox.confirm('删除数据后无法恢复，您确认删除嘛？','删除确认', {type:"warning"}).then(() => {
    request.delete('/coach/deleteBatch', { data: data.ids }).then(res => {
      if(res.code === '200'){
        ElMessage.success('操作成功')
        load() //删除后要重新加载最新数据
      } else {
